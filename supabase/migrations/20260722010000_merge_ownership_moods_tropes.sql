-- Manual merge (docs/task-manual-merge.md §2) — complete the merge_books RPC for the signals added
-- after it was last written. Three gaps, all data-loss or a dropped union on merge:
--
--   1. ownership (four-state, ownership-v2) was NEVER applied from p_fields — the core engine unions
--      it correctly (owned > borrowed > wishlist > unset) but the RPC dropped it, so the primary kept
--      its own ownership regardless. Now applied.
--   2. book_tropes were not re-parented — the loser's tropes died with it on the cascade delete.
--   3. book_moods (reader-assigned, #74) were not re-parented — same loss.
--
-- The core mergeBooks (@reverie/core) still computes the field union; this only closes the RPC's side
-- (ownership apply + join re-parent). Recreated verbatim from taxonomy_neutral otherwise.
...
begin
  ...
  -- 4. Apply the merged scalar/array fields to the primary. ownership is the four-state union the
  --    core engine computed (owned > borrowed > wishlist > unset) — now actually written.
