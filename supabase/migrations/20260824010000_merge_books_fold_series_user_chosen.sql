-- merge_books: fold `series_user_chosen` when the loser's series wins the merge.
--
-- ── THE GAP ─────────────────────────────────────────────────────────────────────────────────────
-- `series_user_chosen` (20260818010000) is provenance for `books.series`: it means the reader named
-- or cleared this series themselves, and `enrichmentSeriesFill` (packages/core/src/match.ts) reads
-- it to decide whether an enrichment sweep may offer one. `merge_books`' latest definition
-- (20260815010000) PREDATES that column and references it zero times, so the primary keeps its own
-- flag while `series = coalesce(p_fields ->> 'series', series)` can hand it the loser's value. The
-- survivor's series then describes one row and its provenance describes another.
--
-- ── WHY THIS IS A LATENT DEFECT, NOT A LIVE ONE — AND WHY IT IS STILL WORTH FIXING ──────────────
-- Stated plainly so nobody re-derives it: the BACKLOG entry's stated harm ("a later enrichment
-- sweep could treat it as fill-only and silently replace it") does NOT happen today. Two reasons,
-- both verified: `enrichmentSeriesFill` withholds on `if (book.series) return ''` BEFORE the flag is
-- consulted, so a non-empty series is never replaced whatever the flag says; and every path that
-- empties `books.series` (`sync_book_series`' clear branches, `remove_series_entry`) sets the flag
-- true as it goes. The unreachable state is `flag = false` with `series` empty.
--
-- What is fixed here is the provenance lie itself. The flag is wrong on the row the moment the
-- merge lands, and it stays wrong; the only thing standing between that and a real re-fill is one
-- short-circuit in one consumer. A second consumer of this flag — the series review bucket, or any
-- future "did the reader choose this?" question — would inherit the wrong answer with no warning.
--
-- ── THE RULE ────────────────────────────────────────────────────────────────────────────────────
-- Fold ONLY when the loser's series actually replaced the primary's, and OR rather than assign:
--   (a) loser's series wins + loser flagged        -> survivor true
--   (b) primary's own series survives              -> survivor's flag untouched
--   (c) neither side flagged                       -> stays false
-- Covered by supabase/tests/merge_books_derive_series_test.sql (extended, not a new file).
--
-- `create or replace`, same signature, so 20260801010000's `revoke execute from public` survives
-- (AGENTS.md's standing rule — verified by proacl, never assumed).
