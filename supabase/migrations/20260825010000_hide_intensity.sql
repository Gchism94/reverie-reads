...
-- "suppress, never clear" rule the format flags follow (AGENTS.md, possession). A reader who hides
...
-- No new RPC, so no revoke/grant pass is required (AGENTS.md's `revoke execute` rule does not apply
-- to a plain column).
