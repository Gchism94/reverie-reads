...
- ONBOARDING / FIRST-RUN — DONE, shipped (6578e51 `feat(onboarding): first-run flow`, iterated
  ...
  design/from-claude-design/onboarding/HANDOFF.md (gitignored working-tree export).
- AUTH SCREENS — DONE, shipped (895320d `feat(auth): landing + password/social auth front door`,
  ...
  the gold brand. Design handoff was design/from-claude-design/auth/HANDOFF.md.
...
- SKIN GALLERY — already BUILT in code (C3). Prompt exists (CLAUDE_DESIGN_PROMPT_SKIN_GALLERY.md)
...
- WRAPPED — the app ships a private stats dashboard titled "Your Reading, Wrapped"
  ...
  "your reading year as art" experience distinct from the dashboard. Whether the Claude Design
  screens (prompt: CLAUDE_DESIGN_PROMPT_WRAPPED.md) ever came back is unverifiable from the tree —
  design/from-claude-design/ is gitignored except its README. If the screens exist in Greg's
...
- COVER STUDIO — per-book cover picking shipped (components/CoverPicker.tsx); the studio surface —
  ...
  component. PROMPT WRITTEN -> design/CLAUDE_DESIGN_PROMPT_COVER_STUDIO.md. Scope in
