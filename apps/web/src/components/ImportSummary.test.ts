...
const base: ImportExportResult = {
  ...
  truncatedIsbns: 0,
  bookIds: [],
  extras: { tbrPlaced: 0, shelvesCreated: [], shelved: 0, noCover: 0, noIsbn: 0, unplacedNotes: 0, tropeLikeShelves: [] },
}
...
describe('ImportSummary copy', () => {
  it('headline states what the import did', () => {
    expect(summaryHeadline(base)).toBe('Detected your generic export — brought in 40 new, and folded 3 into what you had.')
  ...
  it('speaks to the TBR, shelves, missing covers, and unplaced notes — pluralized', () => {
    const lines = summaryNotices(
      withExtras({ tbrPlaced: 12, shelvesCreated: ['Imported TBR', 'Dark Romance', 'Fae'], shelved: 60, noCover: 142, unplacedNotes: 1 }),
    )
  ...
  it('notes trope-like shelves without converting, and truncated ISBNs', () => {
    ...
    expect(lines.find((l) => l.includes('look like tropes') && l.includes('kept as shelves'))).toBeTruthy()
    expect(summaryNotices({ ...base, truncatedIsbns: 3 }).find((l) => l.includes('3 ISBNs may be missing'))).toBeTruthy()
  })
