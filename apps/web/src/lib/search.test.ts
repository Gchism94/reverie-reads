...
import { dedupeResults, libraryMatch, resultKey, resultToIncoming, type SearchResult } from './search'
...
describe('resultKey', () => {
  ...
  it('falls back to normalized title + first author', () => {
    expect(resultKey(result({ title: 'The Fourth Wing', authors: ['Rebecca Yarros'] }))).toBe('the fourth wing|rebecca yarros')
  })
})
...
describe('dedupeResults', () => {
  it('collapses the same book from Hardcover + Google', () => {
    const out = dedupeResults([
      result({ source: 'hardcover', title: 'Fourth Wing', authors: ['Rebecca Yarros'], isbn13: '9781649374042' }),
      result({ source: 'google', title: 'Fourth Wing', authors: ['Rebecca Yarros'], isbn13: '9781649374042' }),
      result({ source: 'google', title: 'Iron Flame', authors: ['Rebecca Yarros'], isbn13: '9781649374172' }),
    ])
...
describe('resultToIncoming', () => {
  it('splits the author and carries ISBN + series', () => {
    const inc = resultToIncoming(result({ title: 'Zephyr', authors: ['Imogen Vale'], isbn13: '9780316580792', series: 'Windborne', seriesPosition: 2 }))
    expect(inc).toMatchObject({ title: 'Zephyr', first: 'Imogen', last: 'Vale', isbn: '9780316580792', series: 'Windborne', position: 2 })
  })
})
...
describe('libraryMatch (de-dupe against the library)', () => {
  ...
  it('matches by ISBN', () => {
    const m = libraryMatch(result({ title: 'Fourth Wing (special ed.)', authors: ['Rebecca Yarros'], isbn13: '9781649374042' }), library)
    expect(m?.id).toBe('lib1')
  })
  it('matches by title + author when no ISBN', () => {
    const m = libraryMatch(result({ title: 'It Ends with Us', authors: ['Colleen Hoover'] }), library)
