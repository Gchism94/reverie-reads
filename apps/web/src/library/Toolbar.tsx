...
function SearchResultsPanel({ q, onPick }: { q: string; onPick: (b: Book) => void }) {
  ...
  const matches = useMemo(() => {
    ...
    return books
      .filter((b) =>
        [b.title, formatAuthors(b.contributors), [b.first, b.last].filter(Boolean).join(' '), b.series]
          .filter(Boolean)
  ...
  return (
    <Frame className="absolute left-0 right-0 top-[calc(100%+6px)] z-30 overflow-hidden p-2 shadow-lg" style={{ boxShadow: 'var(--shadow)' }}>
      ...
      <div className="mt-1 flex items-baseline justify-between border-t border-line px-2 pt-1.5">
        <span className="skin-numeral text-[11px] font-semibold" style={{ color: 'var(--accent-ink)' }}>
  ...
  return (
    ...
    <div className="flex flex-wrap items-center gap-2">
      <div className="relative min-w-[200px] flex-1">
        ...
        />
        {searchFocused && (
          <SearchResultsPanel q={filters.q} onPick={(b) => navigate({ to: '/book/$bookId', params: { bookId: b.id } })} />
        )}
      </div>
