...
export function FilterPanel({ books }: { books: Book[] }) {
  ...
  const tropes = useMemo(() => {
    ...
    for (const b of books) for (const t of b.tropes) counts.set(t, (counts.get(t) ?? 0) + 1)
    const sorted = [...counts.keys()].sort((a, b) => (counts.get(b) ?? 0) - (counts.get(a) ?? 0))
    return { all: sorted, shown: showAllTropes ? sorted : sorted.slice(0, 14) }
  }, [books, showAllTropes])
  ...
  return (
    ...
    >
      ...
      <Group label="Tropes">
        {tropes.shown.map((t) => (
          <Chip key={t} active={filters.tropes.includes(t)} onClick={() => s.toggleTrope(t)}>
            {t}
          </Chip>
        ))}
        {tropes.all.length > 14 && (
          <Chip onClick={() => setShowAllTropes((v) => !v)}>
            {showAllTropes ? 'show less' : 'show all'}
          </Chip>
        )}
      </Group>
