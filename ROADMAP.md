...
|     5 |    P0    | Owner-library backup and CSV reconciliation              | Blocked on catalog-entry review | The first production dry run remained read-only and found 10 exact title/author identities absent from the corpus, so its checksum is not approvable. The current candidate adds a normal household-only catalog path: members may add existing works or create attributed provisional ones; no personal row or ownership claim is invented; only owners/admins edit existing corpus records; corpus changes reach personal copies only through explicit adoption. Independently review, integrate, deploy, and smoke this boundary, resolve the 10 rows through the app, then rerun the dry run before backup or write approval. |
