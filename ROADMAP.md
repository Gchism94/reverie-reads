...
|     3 |    P0    | Prune redundant worktrees and app-related `dev/` folders | In progress   | Resolve the remaining owner-review set: active worktrees, two untracked source files, protected ignored environment material, and the nested locked series checkout. Remove nothing until each item is inactive and its non-cache material has an explicit preservation or discard decision.                                                                                                                                            |
|     4 |    P0    | Build independent, corpus-preserving library membership  | Ready after 3 | Replace the derived household union with first-class household membership and shared enrichment. Personal deletion must preserve household and corpus data; household removal must preserve personal and corpus data. Route genre/subgenre/cover options to the corpus, household tags/tropes to the household overlay, and reading/possession state to personal storage. See `docs/tasks/task-library-removal-and-reconciliation.md`.  |
