...
|     5 |    P0    | Owner-library backup and CSV reconciliation              | Cover blockers in re-review | The first production dry run remained read-only and found 10 exact title/author identities absent from the corpus. PR #367, `20260902010000`, the 51-row production report, and the updated `covers` function are complete; production web is held. Independent review rejected the first `20260903010000` cover fix for implicit unreviewed publication, arbitrary peer hotlinks, and lock inversion. The corrected local candidate uses an admin review switch that starts off, trusted peer projection, and corpus-consistent lock order. It still requires re-review, integration, owner deployment, the 55-row report, and final web smoke before the 10 rows are resolved and reconciliation resumes. |
