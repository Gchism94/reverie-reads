...
Current as of 2026-08-27. This file is the ordered project status. Historical briefs remain in
...
Repository `origin/main` is now `0bd76a5` and includes PR #364's reviewed bounded membership
backfill and corpus-administration history. The owner reports that guarded production deployment
applied migrations `20260830010000` and `20260831010000` in order. Command success is not the
completion gate: the owner-run, read-only postcondition report must still confirm both migration
records, complete bindings, triggers, owner fences, RPCs, and privilege boundaries before private
reconciliation approval begins.
...
|     4 |    P0    | Build independent, corpus-preserving library membership  | ACL fix in review | PR #364 integrated the reviewed bounded backfill and corpus-admin preservation history. After both migrations were applied, the owner-run report found unintended `work_tropes` table grants. Forward migration `20260901010000` resets that ACL to authenticated read-only and service-role write access; it requires independent review and owner deployment before the report can pass. See `docs/queries/library-membership-rollout-verification.sql`, `docs/tasks/task-library-removal-and-reconciliation.md`, and `docs/tasks/task-corpus-admin-enrichment.md`. |
|     5 |    P0    | Owner-library backup and CSV reconciliation              | In progress       | The operator now separates deterministic title-level dry run, read-only snapshot backup, and write into checksum-bound owner approvals. After item 4's read-only report passes and this diff is reviewed, the owner runs and approves the private dry run and exact backup before personally executing the atomic reconciliation. Every resolved CSV book enters the household directly; `TC read` assigns Account A, `GC read` assigns Account B, and unmarked rows remain household-only.                                                                           |
...
- Book edits are scoped: canonical genre/subgenre/cover options update the corpus; household tags,
  ...
  rating, notes, progress, favourites, and lists remain personal. A service-granted corpus
