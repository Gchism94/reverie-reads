...
Repository `origin/main` is now `778131c` and includes PR #361's independent library-membership
foundation plus PR #363's visual facelift. Migration `20260830010000` is intentionally not recorded
in production yet: its original backfill was stopped after the owner observed an unbounded deploy,
and the reviewed bounded-backfill hotfix `c9f07785` must be integrated before that migration is run.
...
|     4 |    P0    | Build independent, corpus-preserving library membership  | Merged; rollout blocked | PR #361 merged the first-class personal/household membership model and removal paths. Integrate the reviewed bounded-backfill hotfix, deploy migration `20260830010000` through the owner-confirmed guard, then complete the corpus-admin preservation follow-up. See `docs/tasks/task-library-removal-and-reconciliation.md` and `docs/tasks/task-corpus-admin-enrichment.md`.                                                                                                                               |
|     5 |    P0    | Owner-library backup and CSV reconciliation              | Ready for review        | The private CSV audit and dry-run-by-default operator are implemented and locally verified without committing reader data. After item 4 is deployed and this diff is independently reviewed, export the affected Account A/B rows, approve the exact title-level external dry-run and backup, then let the owner execute the atomic reconciliation. Every resolved CSV book enters the household directly; `TC read` assigns Account A, `GC read` assigns Account B, and unmarked rows remain household-only. |
