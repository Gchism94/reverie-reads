...
|     7 |    P1    | Series truth and library-experience overhaul             | Phase 1 research       | Code-path research may proceed while item 5 awaits its owner-only write, but no series migration or production-data change bypasses that P0 gate. Audit ingestion and current production-shaped data; default to no series unless membership is sourced or reader-confirmed; preserve provenance and reader edits; reconcile the old string and structured models; then redesign how series appear and are edited in Library. The code-path audit and aggregate-only owner query are staged; see `docs/audits/series-truth-phase-1.md` and `docs/backlog/task-series-truth-library-overhaul.md`.                                                           |
