...
|     5 |    P0    | Owner-library backup and CSV reconciliation              | Awaiting catalog-entry re-review | The first production dry run remained read-only and found 10 exact title/author identities absent from the corpus, so its checksum is not approvable. The locally remediated candidate adds a normal household-only catalog path: members may add existing works or create attributed provisional ones; no personal row or ownership claim is invented; only owners/admins edit existing corpus records; corpus changes reach personal copies only through explicit adoption. Independent re-review, integration, deployment, and smoke verification remain required before resolving the 10 rows through the app and rerunning the dry run. |
