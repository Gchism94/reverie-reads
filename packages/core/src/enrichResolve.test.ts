...
const cand = (source: ResolveCandidate['source'], record: SourceRecord): ResolveCandidate => ({ source, record })
...
describe('scoreCandidate — confidence tiers', () => {
  ...
  it('exact title + author → high', () => {
    expect(scoreCandidate(q, cand('hardcover', { title: 'King of Wrath', authors: ['Ana Huang'] })).confidence).toBe('high')
  })
  it('exact title, no author given → medium (nothing to confirm)', () => {
    expect(scoreCandidate({ title: 'King of Wrath' }, cand('google', { title: 'King of Wrath', authors: ['Ana Huang'] })).confidence).toBe('medium')
  })
  it('close title (subtitle dropped) + author → medium', () => {
    const s = scoreCandidate(q, cand('google', { title: 'King of Wrath: A Novel', authors: ['Ana Huang'] }))
    expect(s.confidence).toBe('medium')
    expect(s.titleExact).toBe(false)
  })
  it('exact title but DIFFERENT author → low (common-title / wrong-book risk)', () => {
    expect(scoreCandidate(q, cand('openlibrary', { title: 'King of Wrath', authors: ['Some Other Person'] })).confidence).toBe('low')
  })
  it('close title only (no author confirm) → low', () => {
    expect(scoreCandidate({ title: 'King of Wrath' }, cand('google', { title: 'King of Wrath: A Novel', authors: ['Anyone'] })).confidence).toBe('low')
  })
  it('unrelated title → none', () => {
    expect(scoreCandidate(q, cand('google', { title: 'Wuthering Heights', authors: ['Ana Huang'] })).confidence).toBe('none')
  })
  it('series match corroborates a close+author medium up to high', () => {
    const s = scoreCandidate(
      ...
      cand('hardcover', { title: 'King of Wrath: A Novel', authors: ['Ana Huang'], series: 'Kings of Sin' }),
    )
...
describe('selectBestMatch', () => {
  it('picks a single exact match, self-resolves the ISBN, returns high', () => {
    const r = selectBestMatch(
      { title: 'King of Wrath', author: 'Ana Huang' },
      [cand('hardcover', { title: 'King of Wrath', authors: ['Ana Huang'], isbn10: '1735056251', cover: 'a.jpg' })],
    )
  ...
  it('same work from two sources is corroboration, not ambiguity (stays high)', () => {
    const r = selectBestMatch(
      { title: 'King of Wrath', author: 'Ana Huang' },
      [
        cand('openlibrary', { title: 'King of Wrath', authors: ['Ana Huang'], isbn13: '9781735056258', cover: 'ol.jpg' }),
        cand('hardcover', { title: 'King of Wrath', authors: ['Ana Huang'], isbn13: '9781735056258', cover: 'hc.jpg' }),
      ],
    )
    expect(r.confidence).toBe('high')
  ...
  it('an author disambiguates a shared title (the matching author wins, stays high)', () => {
    const r = selectBestMatch(
      { title: 'Twisted', author: 'Ana Huang' },
      [
        cand('google', { title: 'Twisted', authors: ['Ana Huang'], isbn13: '9780000000001' }),
        cand('openlibrary', { title: 'Twisted', authors: ['Lisa Jackson'], isbn13: '9780000000002' }),
      ],
  ...
  it('keeps other editions of the same work as alternates (different ISBN, has a cover)', () => {
    const r = selectBestMatch(
      { title: 'King of Wrath', author: 'Ana Huang' },
      [
        cand('hardcover', { title: 'King of Wrath', authors: ['Ana Huang'], isbn13: '9780000000001', cover: 'special.jpg' }),
        cand('google', { title: 'King of Wrath', authors: ['Ana Huang'], isbn13: '9780000000002', cover: 'paperback.jpg' }),
      ],
    )
