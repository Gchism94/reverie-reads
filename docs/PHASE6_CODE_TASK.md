# Phase 6 — Generalize + Skins — Claude Code task

Paste into Claude Code. Builds on the shipped app (Phases 1–5 complete). Same working
...
that approach in production. docs/SKINS.md has the token contract, the cleared Tier-1 names, and
...
- A4 quota: give bulk "fill missing covers & info" a per-run/day ceiling that respects Google
  Books (~1000/day per docs/SCALING.md) and Open Library per-IP courtesy, and degrade gracefully
...
The app is no longer romance-specific; romance becomes the default *skin* (Reverie). Generalize
...
- UI: a skin picker in Settings (separate control from light/dark) + the Skin Gallery screen
  (browse/preview/select; the Claude Design pass is producing the visual for this).
...
- Monthly evolution: a cron Edge Function recomputes the profile; if it shifts materially, surface
  ...
  Claude Design — scaffold against the spec, slot visuals when they land).
...
removed: keep Tropes/Spice as *labels* via skin config. Keep bundles ≤ the cost ceilings noted in
docs/SCALING.md (shared caches keyed by work, not per-user).
