...
small set of _character_ tokens (mode-independent) in a `[data-skin='<id>']` block in
...
The levers that make a skin a _place_, not a recolour. Same discipline: neutral defaults render
...
**AA INVARIANT (the 1b headline):** texture never buries text. The ambient texture lives _behind_
...
**Spine** (`SpineStyle`) is the first _signature component_: a book spine composed for its own narrow,
...
index-rule | dotted`, `frame` +`margin-rule | stitched-inset | record-card | sticker-ring`, `tag` +`drawn-mark | jar-label | index-tab | puffy-sticker`, `progress` +`page-lines | cross-stitch |
