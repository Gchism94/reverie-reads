...
The skins adopted the **paint, not the bone**. On the real app screens, every skin renders the *same boxes in
the same places at the same sizes* — same hero card, same nav, same goal-ring slot, same empty-state panel —
...
**Root cause**: the skin contract is a *styling* contract — it carries values (radius, type, color, texture,
...
## The architecture — extend the contract from *values* to *slots + values*
...
- **Declarative structural config for the common slots.** Most "structural" differences are *parameters*, not
...
Honest tradeoff: this ships per-skin *structure* (config + one component), not just CSS values, so a new skin
...
Retrofit the highest-visibility *composed* surfaces — **the home hero card, one Library section (header + card
...
- **Fill the structural table for the other skins.** Where a skin's structure is inventable in code, code-first;
  ...
  routing as the flat-skin rescue loop, and this is very likely *why* some skins read flat: no bone.
