...
Per CLAUDE.md's "Possession is five independent flags" section, the load-bearing
...
find-or-create in `series.ts`, see CLAUDE.md's "Series-name dual home" rule referenced by
...
not on assumption. CLAUDE.md, the section rules-in-feedback memory, and the Iron-Flame
...
1. `20260812xxxxxx_merge_duplicate_series_*.sql` — re-point orphan `series_entries` to
   ...
   logic, with the same `create or replace` discipline (CLAUDE.md: a function's revoke
   survives `replace`).
...
Phase 3 may show many series carry defects. Per CLAUDE.md: "do not propose automated
