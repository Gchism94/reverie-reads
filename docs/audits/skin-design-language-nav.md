...
- FAB: `var(--shadow)` + a hardcoded `linear-gradient(135deg, var(--primary), var(--gold))` (370-375). The gradient _recipe_ is M1; its two stops are M3.
...
- Nav links / tab links: `transition-colors` (46, 258) with no explicit duration — Tailwind's default 150ms `cubic-bezier(0.4,0,0.2,1)`, again outside the motion tokens. (The one nav element that _does_ ride skin motion is the Add button, via `.skin-control` — see §4.)
...
`.skin-control`/`.skin-btn-primary` sets. The per-skin values that change the nav's _voice_ (from
...
So the nav's per-skin _typography_ is fully tokenized; its per-skin _shape and motion_ are not —
...
The brand block is the one place the nav is _materially_ different per skin. Three coordinated
pieces:
...
1. **A carved object ornament.** The lintel (skin-kit.css:496-509) is a solid with a gradient face
   and a chamfer _cut into its silhouette_ (clip-path), hung under the wordmark. No body surface
...
4. And the inversions, stated for completeness: Marrow's nav _items_ are rounder (10px, M1) than
