...
> **Archived: brief complete; infrastructure live.** The task brief is done — Stage 1 merged
> (`913ba22`), Stage 2 merged (`de7e712`), Stage 3 measured and cancelled with a recorded reason.
> Archiving the **brief** does not archive the **infrastructure**: `.github/workflows/ci.yml`
> and `cla.yml` keep running on every push and PR regardless of where this originating task doc
> lives. A future reader should not read "this task is archived" as "this CI setup is deprecated."
> For current CI behavior, read the workflow files directly.
