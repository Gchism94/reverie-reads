...
top to bottom; commit at each checkpoint. Full context is in `CLAUDE.md` and `docs/`.

## Paste-to-start prompt

> Read `CLAUDE.md` and `docs/archive/CLAUDE_CODE_KICKOFF.md`. Then do Step 1 (scaffold) and stop
...
- ESLint + Prettier + Vitest + Playwright configured; the `pnpm` scripts in `CLAUDE.md`.
