...
> **Archived: superseded.** This task shipped in #61 under a proprietary / all-rights-reserved
> license decision. That decision was **reversed** by `chore/pre-public-licensing`, which split
> the seed and dedicated the bibliographic corpus (`data/corpus_seed.json`) under CC0. The license
> posture described below is **no longer current**. Kept as historical record of the brief the
> #61 work was built against. For the current license posture, read `LICENSE` and `LICENSE-CORPUS`
> at repo root.
...
--json`, not edited by hand. For current behavior, read the code and `docs/reference/DATA_MODEL.md`.
...
The current README is build-phase oriented and points at `CLAUDE.md`. Rewrite for a
...
- **Deploy discipline:** point to `docs/reference/DEPLOY.md` and state that prod deploys go
